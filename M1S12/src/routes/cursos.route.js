...
cursoRoutes.post('/', auth,async (req, res) => {
    try {
        ...
        if(!(duracao_horas >= 30 && duracao_horas <= 400)) {
            return res.status(400).json({  messagem: "A duração do curso deve ser entre 30 e 400 horas"
            })
        }
        ...
        res.status(500).json({ error: 'Não consegui cadastrar o curso' })
    }
...
cursoRoutes.get('/', auth, async (req, res) => {
    try {
        ...
        console.log(error.message)
        res.status(500).json({ error: 'Não consegui listar os cursos...' })
    }
})
...
cursoRoutes.put('/:id', auth,async (req, res) => {
    ...
    if(!curso) {
        return res.status(404).json({mensage: 'Não consegui localizar o curso!'})
    }
